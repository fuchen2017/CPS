<?php
  ...
  //包含需求檔案 ------------------------------------------------------------------------
	...
	//取出測驗資料
	$sql_dsc = "select * from `opt_record` where `main_data_num` in ('66','71') and `create_user_type`='STUDENT' and `questions_power_dsc`='' order by `num` desc ".$pg_dsc;//管理員資料
	$res=$ODb->query($sql_dsc) or die("載入資料出錯，請聯繫管理員。");
	$hasData = false;
	if(count($res) > ''){
	$hasData = true;		
...
<?php
 if($hasData){ 
 ?>	
	var delTime = 300000;//每五分鐘觸發一個 300000
	var allTime = 1000;
    $('a[subid="target"]').each(function( index ) {
		var xxx = $(this).attr('href');
		setTimeout(function() {
			window.open(xxx);
		}, allTime);
		allTime = allTime + delTime;
	});
	setTimeout(function() {
			location.reload();
		}, allTime);
    ...
    <div class="accordionButton">
        <table class="list_item">
            <tr>
                ...
                <td width="18%">
                    <a class="button" title="觀看操作步驟" href="<?php echo $module_type_array[$test_data_array[$x]['main_data_num']];?>?m_num=<?php echo $test_data_array[$x]['main_data_num'];?>&num=<?php echo $test_data_array[$x]['num'];?>" subid="target">觀看操作步驟</a>                
                </td>
            </tr>
        </table>             
    </div>
