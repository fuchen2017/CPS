<?php
	...
	include("./bcontroller/module_function/science.php");//科學模組
	...
	while($row = mysql_fetch_array($res)){
		if($row['c_user_type'] == 0){//使用者對話
			$dsc_0.= "<li><input type='radio' subid='talk_radio' onclick=\"get_last_pcmsg('".$row['num']."')\"  id='speech_radio_".$num."' value='".$row['num']."' name='speech_radio' ><label for='speech_radio_".$num."'>".$row['c_dsc']."</label></li>";
			$num++;
...
<script src="./js/selectToUISlider.jQuery.js"> </script><!-- 選擇刻度 -->
<script src="./js/jquery.colorbox.js"> </script>
<script type="text/javascript" src="./js/jcanvas.js"> </script><!--繪圖區可拖曳元件-->
...
var module_7_array = new Array();//水淨化模組最後一道順序的陣列
...
var ajax_obj = "get_operation_data.php";
...
//產生水模組最後一筆資料的回顧按鈕
function show_module_7_final(){
	var dsc_array = new Array('一','二','三','四','五','六');
	for(var x=0;x<module_7_array.length;x++){
	var btn_dsc ='<button type="button" class="s7_btn" onclick="chg_module_7_4_img('+x+')">第'+dsc_array[x]+'次評分</button>';
	$('#module_7_final').append(btn_dsc);
	}
	$('#module_7_final').show();
}
function chg_module_7_4_img(getindex){
$('#module_7_4 img').attr("src",module_7_array[getindex]);
set_record('science||s_seven||chg_img||'+getindex);
}
...
function set_select(getKey){
	if($('#'+getKey).attr('class') == ""){
		$('#'+getKey).attr('class','select');
	}else{
		$('#'+getKey).attr('class','');
	}
}
